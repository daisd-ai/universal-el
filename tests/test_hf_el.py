...
from src.hf_el import MemoryEfficientLinker

FAISS_INDEX_PATH = "Faiss Wikidata index PATH GOES HERE"
TEST_FILE_PATH = os.path.join("tests", "files", "tests_Tweeki_gold.jsonl")


class TestMemoryEfficientLinker(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.el = MemoryEfficientLinker(faiss_index_path=FAISS_INDEX_PATH)
